...
def make_dot(var, params=None):
    ...
    node_attr = dict(style='filled',
                     ...
                     ranksep='0.1',
                     height='0.2')
    dot = Digraph(node_attr=node_attr, graph_attr=dict(
        size="4,4"
        #rankdir="LR"
        ))
    seen = set()
...
def get_net_svg(net, device, input_shape):
    dummy_input = torch.zeros(input_shape).to(device)
    dummy_output = net(dummy_input)
    dot = make_dot(dummy_output)
    dot.format = 'svg'
    return str(dot.pipe())

def get_model_svgs(model, input_shapes):
    svg_strings = dict.fromkeys(input_shapes.keys(), None)
    for name, net in model.nets.items():
        svg_string = get_net_svg(net, model.device, input_shapes[name])
        svg_strings[name] = svg_string
    return svg_strings
